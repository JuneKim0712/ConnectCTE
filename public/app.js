...
document.getElementById('addPartnerBtn').addEventListener('click', () => document.getElementById('partnerForm').hidden = false);

document.getElementById('closeFormBtn').addEventListener('click', () => {
    document.getElementById('partnerForm').reset();
    document.getElementById('partnerForm').hidden = true;
});
...
document.getElementById('partnerForm').addEventListener('submit', (e) => {
    ...
    add(thingsRef, name, type, sector, resources, individual, email, phone, address, date);
    document.getElementById('partnerForm').reset();
    document.getElementById('partnerForm').hidden = true;
});
...
function add(thingsRef, name, type, sector, resources, individual, email, phone, address, date) {
    thingsRef.add({ name, type, sector, resources, individual, email, phone, address, date });
}

function initDb(thingsRef, example) {
    thingsRef.get()
        .then(querySnapshot => querySnapshot.forEach(doc => doc.ref.delete()))
        .then(() => example.forEach(i => add(thingsRef, ...i)))
...
const filterPartners = (typeFilter, sectorFilter, searchInput) => {
    ...
    return query.onSnapshot(snapshot => {
        ...
        snapshot.forEach(doc => {
            ...
            if (!searchInput || Object.values(partnerData).some(value => value.toLowerCase().includes(searchInput))) {
